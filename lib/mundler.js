    ...
    return Promise.all(Object.keys(bundles)
      ...
      .map(function(bundle) {
        ...
        return self.verifyRequiredProps(bundle, bundleProps).spread(function(name, props) {
          ...
          if (!!props.watch) {
            logger.log('Watching "'+name+'" for changes...\n');
          }
  ...
  return vendorBundle.then(function() {
    return self.glob(src).then(function(filesArr) {
      return Promise.all(filesArr
        ...
        if (!props.watch) {
          logger.log('Building "'+ bundleName +'" bundle...');
        }
...
Mundler.prototype.configureVendorBundle = function(bundleName, props, modules, aliases) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (!modules || !modules.length) {
      ...
      self.vendorBundleCache['vendor-'+bundleName] = '';
...
Mundler.prototype.taskFunctionCallback = function(resolve, reject) {
  return function(err) {
    if (err) {
      logger.log(chalk.red('Function task failed'));
      return reject(err);
    }
...
Mundler.prototype.taskRunCommand = function(task) {
  return new Promise(function(resolve, reject) {
    logger.log('Running task "'+task+'"');

    var run = exec(task);
    run.stdout.pipe(process.stdout);
    run.on('close', function(code) {
      if (code !== 0) {
        logger.log(chalk.red('Task "'+task+'" failed'));
        reject('');
      }
      else {
        logger.log('Task "'+task+'" completed successfully\n');
        resolve();
      }
    });
  });
  ...
  b.on('update', function(ids) {
    ...
    self.readFile(file).then(function(data) {
      ...
      if (!change) {
        ...
        logger.log(chalk.green('Detected '+change+'"... rebuilding before bundling "'+name+'"'));
      ...
      self.runTasks(preTasks).then(function() {
        b.bundle(function(err, buf) {
          if (!!props.concat) {
            if (name.indexOf('vendor') === -1 && !!self.vendorBundleCache['vendor-'+name] && !!cachedModules && cachedModules.length) {
          ...
          if (!!self.streams) {
            ...
            fs.writeFile(dest, buf, function(err) {
              ...
              logger.log(chalk.yellow('Bundle "'+name+'"') + ': written in %s seconds\n', (end - start) / 1000);
    ...
    self.runTasks(preTasks).then(function() {
      b.bundle(function(err, buf) {
        ...
        if (!writeToFile) {
          ...
          logger.log(chalk.yellow('Bundle "'+name+'"') + ': created in %s seconds and cached for concatenation\n', (end - self.start) / 1000);
          return resolve(buf);
        }
        ...
        if (!!self.streams) {
          ...
          fs.writeFile(dest, buf, function(err) {
            ...
            var end = new Date().getTime();
            logger.log(chalk.yellow('Bundle "'+name+'"') + ': written in %s seconds\n', (end - self.start) / 1000);
            self.runTasks(postTasks).catch(catchError);
            resolve(buf);
          });
        }
