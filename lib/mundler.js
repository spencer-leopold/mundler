//
// @TODO: Make start method return a promise
// @TODO: Better require/import finding. Prevent finding in comments, etc. Regex to strip comments: /(/\*([^*]|[\r\n]|(\*+([^*/]|[\r\n])))*\*+/)|(//.*)/g
// @TODO: create tally of already read files, to prevent reading multiple times
// if the file is required in another file
// @TODO: Add better log messages
//
...
Mundler.prototype.bundle = function() {
  ...
  return browserAliases.then(function(aliases) {
    return browserShims.then(function(shims) {
      ...
      return Promise.all(Object.keys(options)
        ...
        .map(function(bundle) {
          ...
          return self.verifyRequiredProps(bundle, bundleProps).then(function(config) {
            ...
            return self.buildDependencyList(name, props).then(function(config) {
              return self.configureBundle(config.name, config.props, config.modules, aliases).then(function(buf) {
                return bundles[config.name] = buf;
              });
...
Mundler.prototype.verifyRequiredProps = function(bundleName, props, callback) {
  return Promise.promise(function(resolve, reject) {
    if (!props.hasOwnProperty('src')) {
      ...
      resolve({ name: bundleName, props: props });
    }
  });
}
  ...
  return self.configureVendorBundle('vendor-'+bundleName, props, modules, aliases).then(function() {
    return self.glob(src).then(function(filesArr) {
      return Promise.all(filesArr.map(function(file) {
        ...
        return self.createBundle(b, bundleName, dest, props).then(function(buf) {
          return buf;
        }).catch(console.log.bind(console));
...
Mundler.prototype.configureVendorBundle = function(bundleName, props, modules, aliases) {
  ...
  return Promise.promise(function(resolve, reject) {
    ...
    if ((!modules || !modules.length) && (!aliases || !Object.keys(aliases).length)) {
      ...
      self.createBundle(b, bundleName, dest, props).then(function(buf) {
        resolve(buf);
      }).catch(console.log.bind(console));
    }
  });
