...
Mundler.prototype.configureBundle = function(bundleName, props, modules, aliases) {
  ...
  if (!modules || !modules.length) {
    vendorBundle = self.configureVendorBundle('vendor-'+bundleName, props, modules, aliases);
  }
  else {
    if (!props.watch) {
 ...
 * @param {Object} aliases 
 *
 * Note: if modules is a string we want to get the full module list from the bundle's module cache
 */
Mundler.prototype.configureVendorBundle = function(bundleName, props, modules, aliases, file) {
  var dest, self = this;
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (!modules || !modules.length) {
      ...
      self.vendorBundleCache[bundleName] = '';

      resolve(false);
    }
    else {
      if (typeof modules === 'string') {
        var moduleCache = self.moduleCache[modules];
        modules = [];

        Object.keys(moduleCache).forEach(function(filename) {
          var moduleList = moduleCache[filename];
      ...
      modules.forEach(function(module) {
        ...
        if (!self.browserShims[module]) {
          if (!!aliases && !!aliases[module]) {
            var filePath = path.resolve(aliases[module]);
      ...
      b.bundle(function(err, buf) {
        ...
        if (!!self.streams) {
          self.vendorBundleCache[bundleName] = buf;
          ...
          if (!!props.concat) {
            ...
            self.vendorBundleCache[bundleName] = buf;
            self.logger.log(chalk.yellow('Bundle "'+bundleName+'"') + ': created in %s seconds and cached for concatenation\n', (end - self.start) / 1000);
          ...
          fs.writeFile(dest, buf, function(err) {
            ...
            var end = new Date().getTime();
            self.logger.log(chalk.yellow('Bundle "'+bundleName+'"') + ': written in %s seconds\n', (end - self.start) / 1000);
...
Mundler.prototype.writeToFileOrStream = function(b, name, dest, props) {
  var self = this;
  var writeToFile = true;
    ...
    self.runTasks(preTasks).then(function() {
      b.bundle(function(err, buf) {
        ...
        if (!!self.streams) {
          ...
          stream.put(buf, 'utf8');
          var gulpRet = { file: path.basename(dest), stream: stream };
          self.emit('update', gulpRet);
          resolve(gulpRet);
        }
        else {
          if (!writeToFile) {
            ...
            fs.writeFile(dest, buf, function(err) {
              ...
              self.runTasks(postTasks).catch(self.logger.log);
              resolve(buf);
            });
          }
        }
  ...
  b.on('update', function(ids) {
    ...
    self.checkFileForChanges(file, name)
      ...
      .then(function() {
        ...
        return self.writeToFileOrStream(b, name, dest, props);
      }).catch(function(e) {
