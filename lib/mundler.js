...
Mundler.prototype.searchForDependencies = function(files, props, moduleList) {
  ...
  return Promise.all(files.forEach(function(file) {
    return self.readFile(file).then(function(data) {
      var matches = [];
      for (var m = null; m = regex.exec(data); matches.push(m[1]));

      return Promise.all(matches.forEach(function(match) {
...
Mundler.prototype.configureBundle = function(bundleName, props, modules, aliases) {
  ...
  self.configureVendorBundle('vendor-'+bundleName, dest, concat, modules, aliases).then(function() {
    glob(src, function(err, filesArr) {
      if (err) {
        console.log(err);
        return false;
 ...
 * @param {String} bundleName
 * @param {String} mainDest
 * @param {Boolean} concat
 * @param {Array} modules
 * @param {Object} aliases 
 */
Mundler.prototype.configureVendorBundle = function(bundleName, mainDest, concat, modules, aliases) {
  var self = this;
