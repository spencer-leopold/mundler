//
// @TODO: If watch option is used, need to check changed file for any
// new/removed external dependencies and rebuild the vendor script if
// need be.
//
// @TODO: If tasks is an array, each one needs to return a promise
// the way runTasks is implemented now won't be reliable if there are
// multiple tasks passed in - might even fail since resolve will have already
// been called
//
...
Mundler.prototype.runTasks = function(tasks) {
  return new Promise(function(resolve, reject) {
    ...
    if (typeof tasks === 'string') {
      ...
      console.log('Running task "'+tasks+'"');
      ...
      run.on('close', function(code) {
        if (code !== 0) {
          ...
          console.log('Task "'+tasks+'" completed successfully\n');
    ...
    self.readFile(file).then(function(data) {
      ...
      return Promise.all(matches
        ...
        if (matches.length < moduleCache.length) {
          ...
          return 'REMOVAL of external dependencies';
        }

        // dependency added
        if (matches.length > moduleCache.length) {
          self.moduleCache[name][file] = matches;
          return 'NEW external dependencies';
        }
      ...
      if (!change) {
        ...
        return Promise.all(Object.keys(self.moduleCache[name])
          ...
          return Promise.all(tempModuleList
            .filter(function(m) {
              return (modulesList.indexOf(m) === -1);
            })
            .map(function(m) {
              modulesList.push(m);
              return m;
