  ...
  return Promise.promise(function(resolve, reject) {
    glob(src, function(err, filesArr) {
      self.searchForDependencies(filesArr, props).then(function(modules) {
        resolve({ bundle: bundleName, props: props, modules: modules });
      });
    });
  });
}

Mundler.prototype.buildBundle = function(bundleName, props, modules, aliases, shims) {
  var self = this;
  ...
  if (modules && modules.length) {
    ...
    self.buildVendorBundle('vendor-'+bundleName, props, modules, aliases);
  }
...
Mundler.prototype.buildVendorBundle = function(bundleName, props, modules, aliases) {
  var self = this;
  var src = props.src;
  var dest = path.dirname(props.dest) + '/' + bundleName + '.js';
  var cwd = props.cwd || processCwd;
  var b = browserify();
