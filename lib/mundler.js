...
Mundler.prototype.configureVendorBundle = function(bundleName, props, modules, aliases) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if ((!modules || !modules.length) && (!aliases || !Object.keys(aliases).length)) {
      ...
      self.createBundle(b, bundleName, dest, props)
        .then(resolve)
        .catch(catchError);
    }
  });
    ...
    if (typeof tasks === 'string') {
      ...
      run.on('close', function(code) {
        if (code !== 0) {
          ...
          console.log(chalk.italic(
            'Task "'+tasks+'" completed successfully\n'
          ));
...
Mundler.prototype.createBundle = function(b, name, dest, props) {
  ...
  if (name.indexOf('vendor') === -1) {
    preTasks = props.preTasks || false;
    postTasks = props.postTasks || false;
  }
    ...
    self.runTasks(preTasks).then(function() {
      b.bundle(function(err, buf) {
        ...
        if (!writeToFile) {
          ...
          console.log(chalk.yellow('Bundle "'+name+'"') + ': created in %s seconds\n', (end - self.start) / 1000);
          return resolve(buf);
        }
